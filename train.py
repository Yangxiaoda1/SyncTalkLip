...
from fairseq.dataclass.utils import convert_namespace_to_omegaconf, merge_with_parent#, populate_dataclass
...
class Talklipdata(object):
    ...
    def croppatch(self, images, bbxs):
        patch = np.zeros((images.shape[0], 96, 96, 3), dtype=np.float32)#存储裁剪后的图像[图像数量, 96, 96, 3]
        width = images.shape[1]
        for i, bbx in enumerate(bbxs):#0：左，1：上，2：右，3：下
            bbx[2] = min(bbx[2], width)# 确保边界框的右边界不超过图像的宽度
            bbx[3] = min(bbx[3], width)# 确保边界框的下边界不超过图像的宽度
            patch[i] = cv2.resize(images[i, bbx[1]:bbx[3], bbx[0]:bbx[2], :], (self.crop_size, self.crop_size))
        return patch#对第i张剪裁，并调整到crop_size*crop_size大小
    ...
    def __getitem__(self, idx):
        ...
        # print("\033[92m"+"{}".format(bbx_path)+"\033[0m")
...
def save_sample_images(x, g, gt, global_step, checkpoint_dir):
    ...
    refs, inps = x[..., 3:], x[..., :3]# 分离参考图像和输入图像
    folder = join(checkpoint_dir, "samples_step{:09d}".format(global_step))
    if not os.path.exists(folder): os.mkdir(folder)
    collage = np.concatenate((refs, inps, g, gt), axis=-2)# 将参考图像、输入图像、生成图像、真实图像按宽度方向拼图
    ...
    for epoch in range(args.n_epoch):
        ...
        for step, (inpim, spectrogram, audio_idx, gtim, ((trgt, prev_trg), tlen, ntoken), padding_mask, vidx, videos, bbxs) in prog_bar:
            ...
            try:
                ...
                # if sample['net_input']['source']['video'] is None:
                #     qprint('None','video')
                # else:
                #     qprint('Not None','video')
                # if sample['net_input']['source']['audio'] is None:
                #     qprint('None','audio')
                # else:
                #     qprint('Not None','audio')
                _, _, _, sync_enc_audio = criterion(avhubert, sample)
                sync_enc_audio=sync_enc_audio['encoder_out']

                if lip_train:
                    processed_img = images2avhubert(vidx, videos, bbxs, syntim, spectrogram.shape[2], device)
                    sample['net_input']['source']['video'] = processed_img# 更新sample字典中的视频和音频数据
                    sample['net_input']['source']['audio'] = None
                    lip_loss, sample_size, logs, enc_out = criterion(avhubert, sample)# 计算唇形损失
                    
                    losses['lip'] += lip_loss.item()
                    if args.cont_w > 0:
                        pickedVid, pickedAud = local_sync_loss(audio_idx, sync_enc_audio, enc_out['encoder_out'])#计算视频和音频的同步损失
                        local_sync = model['gen'].sync_net(pickedVid, pickedAud)
                        losses['local_sync'] += local_sync.item()
                    else: # 如果不是训练唇形同步，将唇形损失和局部同步损失设置为0
                        local_sync = 0.
                else:
                    lip_loss, local_sync = 0., 0.
                if args.perp_w > 0.:
                    perceptual_loss = model['disc'].perceptual_forward(syntim)#计算对抗损失
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cont_w', help='weight of contrastive learning', default=1e-4, type=float)#原来是1e-3
    parser.add_argument('--perp_w', help='weight of perceptual loss', default=0.07, type=float)

    # training
    parser.add_argument('--gpu', help='index of gpu used', default=3, type=int)
    ...
    parser.add_argument('--ckpt_interval', help='The interval of saving a checkpoint', default=5000, type=int)
    ...
    test_dataset = Talklipdata('val', args, label_proc)
    ...
    imGen = TalkLip(encoder, 1024).to(device)#原768->我改成了1024
