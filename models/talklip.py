...
class TalkLip(nn.Module):
    def __init__(self, audio_encoder, audio_num, res_layers=None):
        ...
        # qprint(audio_encoder,'audio_encoder')
        self.audio_encoder = audio_encoder
        self.audio_map = nn.Linear(audio_num, enc_channel[-1])
        # qprint(audio_num,'audio_num')
        # qprint(enc_channel[-1],'enc_channel[-1]')
    ...
    def forward(self, sample, face_sequences, idAudio, B):
        ...
        with torch.no_grad() if not self.ft else contextlib.ExitStack():
            enc_out = self.audio_encoder(**sample["net_input"])# 对音频编码进行前向传播
        # T*B*C, B*T

        audio_embedding, audio_padding = enc_out['encoder_out'], enc_out['padding_mask']# 得到音频编码和填充掩码

        # qprint(enc_out['audiox'],'audiox')
        feats = []
        x = face_sequences# 处理人脸序列
        # output is N*512*1*1
        for f in self.face_encoder_blocks:# 通过每个人脸编码器块进行前向传播
            x = f(x)
            feats.append(x)
        # qprint(audio_embedding.shape,'audio_embedding.shape')
        # T*B*C -> N*C -> N*512*1*1
        x = audio_embedding.permute(1, 0, 2).reshape(-1, audio_embedding.shape[2])[idAudio]
        # qprint(x.shape,'x.shape')
        x = self.audio_map(x).reshape(x.shape[0], 512, 1, 1)# 重新排列音频编码，然后进行映射
